...
var ExamPaperPage = (agrs) => {
	...
	_template = agrs.template || tplLoader('tote/html/examPaper.html');
	agrs.template = _template;
	
	autoId = _data.extraOptions.autoId;
	fileName = _data.extraOptions.fileName;
	type = _data.type;
	storageId = 'BDOTOTE' + autoId;
	storageStatus = 'BDOTOTE' + autoId + 'Status';
	storageTime = 'BDOTOTE' + autoId + 'Time';
	if(type == '3'){
		height = height + 40;
		if (height <= 0) {
	...
	listener = () => {
		...
		$('#uploadExcelBtn').click((event) => {
			// 考试试卷id
			uploadExamPaperForm.jsonData.autoId = autoId;
			$('#uploadExamPaperFormModal').modal('show');
		});
		/**
		 * 获取最新版本
		 */
					...
					success(data) {
						if (data.success) {
							if(data.data[0].result){
								$.ajax({
									type: 'post',
									url: 'tote/ToteExam.finishToteTplFile.json',
									data: {
										menuId: window.sys_menuId,
										param1: autoId,
										param2: type
									...
									success(data) {
										if (data.success) {
											$('#fileInfoGroup', window.parent.document).find("li[data-nodeid='" + autoId + "']").find('.badge').text('√');
											bdoInProcessingBox('保存中');
											transition(() => {
												var json = spread.toJSON();
												saveExcel(json);
	...
	mount = () => {
		...
		uploadExamPaperForm = createForm(uploadExamPaperCfg);
		listener();
		transition(() => {
			getExcelData({
				menuId: window.sys_menuId,
	...
	saveExcelServer = (param, fileData) => {
		...
		$.ajax({
			url: 'tote/ToteExam.saveToteFile.json',
			type: 'post',
			data: formData,
			contentType: false,
			processData: false,
	...
	getExcelData = (param) => {
		...
		if (fileData == null) {
			$.ajax({
				url: 'tote/ToteExam.getToteFileInfo.json',
				type: 'post',
				data: param,
				dataType: 'json',
				success(data) {
			...
			$.ajax({
				...
				success(data) {
					if (data.success) {
						if (data.success) {
							var time = parseInt(data.data[0].time);
	...
	uploadExamPaperCfg = {
		...
		column: 1,
		id: 'uploadExamPaperForm',
		data() {
			return {
				ajaxConfig: {
					type: 'POST',
					url: 'tote/ToteDownload.noDBuploadExamPaper.json',
					dataType: 'json',
					success: function(data) {
						if (data.success) {
							...
							$('#uploadExamPaperFormModal').modal('hide');
						} else {
							bdoErrorBox('失败', data.resultInfo.statusText);
						}
					}
		...
		methods: {
			onUploadExamPaperFileClick(event) {
		...
		buttons: [{
			...
			id: 'uploadExamPaperFileBtn',
			icon: 'fa fa-upload',
			style: 'btn-primary',
			text: '上传',
			typeAttr: {
				'v-on:click': 'onUploadExamPaperFileClick'
			}
		}, {
			id: 'cancelUploadExamPaperFileBtn',
			icon: 'fa-arrow-left',
			style: 'btn-warning',
			text: '取消',
			typeAttr: {
