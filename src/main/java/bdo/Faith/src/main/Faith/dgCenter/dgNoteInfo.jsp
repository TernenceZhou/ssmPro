...
<script>
	$(document).ready(() => {
		if (window.nodeData == null) {
			window.nodeData = JSON.parse($.sessionStorage('noteInfoNode'));
			window.nodeData.nodeType = 'note';
		}
		if (nodeData) {
			var fileName = nodeData.extraOptions.fileName;
			document.title = fileName.replace('.xlsx', '');
			window.sys_menuId = nodeData.menuId;
			var $div = $('<iframe id="iframe_note_' + nodeData.extraOptions.autoId + '" src="/Faith/dgcenter.do?m=openDgNoteMain&index=' + nodeData.extraOptions.noteNO + '&projectId=' + window.CUR_PROJECTID + '" allowfullscreen="true" scrolling="no" width="100%px" height="100%" style="border: none;"></iframe>');
