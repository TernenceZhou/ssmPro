...
var App = function() {
	...
	var uiInit = function() {
		...
		$lHtml.on('click', '[data-toggle="tabs"] a, .js-tabs a', function(e) {
			e.preventDefault();
			jQuery(this).tab('show');
		});
		...
		$lHtml.on('show.bs.modal', '.modal', function() {
			$(this).draggable({
				handle: '.block-header',
				cursor: 'move'
			});
		...
		$lHtml.on('shown.bs.tab', '[data-toggle="tabs"] a', function(evt) {
			let href = evt.target.href;
			let index = href.lastIndexOf('#');
			let id = href.substring(index + 1);
			/*let tbls = $('#' + id + ' table[id].dataTable');
			if (tbls.length > 0) {
				tbls.dataTable()._fnReDraw();
			}*/
			uiReDrawTable($('#' + id + ' table[id].dataTable'));
		});
	};

	var uiReDrawTable = function($table) {
		if ($table.length > 0) {
			$table.each((index, table) => {
				$(table).trigger('column-sizing.dt.DTFC');
				$(window).trigger('resize.DT-'+table.id);
				$(window).trigger('resize.DTS');
				//$(table).dataTable()._fnReDraw();
			});
		}
	};

	var uiDestroy = function($currentContext, plugin) {
	...
	var uiBlocks = function() {
		...
		$lBody.on('click', '[data-toggle="block-option"]', function() {
	...
	var uiHandleFontSizeTheme = function() {
		var $cssTheme = jQuery('#bdoFontSizeThemes');
		var $cookies = $lPage.hasClass('enable-cookies') ? true : false;

		// If cookies are enabled
		if ($cookies) {
			var $theme = Cookies.get('fontSizeTheme') ? Cookies.get('fontSizeTheme') : false;
			...
			if ($theme) {
				if ($theme === 'default') {
					...
					if ($cssTheme.length) {
						...
						jQuery('#css-main')
							.after('<link rel="stylesheet" id="bdoFontSizeThemes" href="' + $theme + '">');
					}
				}
			}

			$cssTheme = jQuery('#bdoFontSizeThemes');
		}

		// Set the active color theme link as active
		jQuery('[data-toggle="font-theme"][data-theme="' + ($cssTheme.length ? $cssTheme.attr('href') : 'default') + '"]')
			.parent('li')
			.addClass('active');

		// When a color theme link is clicked
		jQuery('[data-toggle="font-theme"]').on('click', function() {
			var $this = jQuery(this);
			var $theme = $this.data('theme');

			// Set this color theme link as active
			jQuery('[data-toggle="font-theme"]')
				.parent('li')
				.removeClass('active');

			jQuery('[data-toggle="font-theme"][data-theme="' + $theme + '"]')
				.parent('li')
				.addClass('active');
			...
			// If cookies are enabled, save the new active color theme
			if ($cookies) {
				Cookies.set('fontSizeTheme', $theme, {expires: 7});
			}
		});
	};
	...
	var loaderIsShow = false;
	var loaderShowTime = null;
	var loaderHideTime = null;
	var loaderShowCount = 0;
	var dataTableProcessing = '';
	/*var uiLoader = function($mode, showTimeout, force) {
		if(loaderHideTime) {
			console.log(loaderFocre + '::loaderHideTime3');
		}
		var $lpageLoader = jQuery('#page-loader');
		var to = showTimeout || 5*1000;
		var hidTo = showTimeout || 500;
		if ($mode === 'show') {
			dataTableProcessing = $.fn.dataTable.defaults.oLanguage.sProcessing;
			$.fn.dataTable.defaults.oLanguage.sProcessing = '';
			loaderShowCount++;
			if ($lpageLoader.length) {
				$lpageLoader.fadeIn(250);
			} else {
				$lpageLoader = $('<div id="page-loader"></div>');
				$lBody.prepend($lpageLoader);
			}
			$lBody.addClass('show-faith-loader');
			loaderIsShow = true;
			if(loaderShowTime != null) {
				clearTimeout(loaderShowTime);
				loaderShowTime = null;
			}

			if(loaderHideTime) {
				clearTimeout(loaderHideTime);
				loaderHideTime = null;
			}
			loaderShowTime = setTimeout(()=>{
				clearTimeout(loaderShowTime);
				loaderShowTime == null;
				uiLoader('hide');
			}, to);
			...
			if ($lpageLoader.length) {
				...
				function tt() {
					if(loaderShowCount == 0) {
						...
						//console.info(arguments.callee.tt, 'hide');
					}
					loaderIsShow = false;
					if(loaderShowTime != null) {
						clearTimeout(loaderShowTime);
					...
					$lBody.removeClass('show-faith-loader');
					$.fn.dataTable.defaults.oLanguage.sProcessing = dataTableProcessing;
					loaderFocre = false;
				}
				tt.tt = new Date().getTime();
				loaderHideTime = setTimeout(tt, hidTo);
				loaderFocre = force || false;
			}
		}
	...
	var uiLoader = function($mode, timeout) {
		var $lpageLoader = jQuery('#page-loader');
		var to = timeout || 5*1000;
		if ($mode === 'show') {
			dataTableProcessing = $.fn.dataTable.defaults.oLanguage.sProcessing;
			$.fn.dataTable.defaults.oLanguage.sProcessing = '';
			loaderShowCount++;
			...
			//console.info(new Date().getTime(), to);
		} else if ($mode === 'hide') {
			if(loaderShowCount > 0) {
				loaderShowCount--;
				if(loaderShowCount < 0) {
			...
			if ($lpageLoader.length) {
				...
				tt.tt = new Date().getTime();
				loaderHideTime = setTimeout(tt, 500);
			}
		}
	...
	var uiHelperSlimscroll = function() {
		...
		let $el;
		let slimscrollContextSelector = $.sessionStorage('slimscrollContext');
		if(slimscrollContextSelector && slimscrollContextSelector > '') {
			let slimscrollContext = $(slimscrollContextSelector);
			if(slimscrollContext.length > 0) {
				$el = jQuery('[data-toggle="slimscroll"]', slimscrollContext);
			}
		}else {
			$el = jQuery('[data-toggle="slimscroll"]');
		}
		...
		$.sessionStorage('slimscrollContext', null);
	};
	...
	var notifyObject = {
		info: {
			icon: notifyIcons.info,
			type: notifyType.info,
			from: notifyFrom.top,
			align: notifyAlign.center
		},
		success: {
			icon: notifyIcons.success,
			type: notifyType.success,
			from: notifyFrom.top,
			align: notifyAlign.center
		},
		warning: {
			icon: notifyIcons.warning,
			type: notifyType.warning,
			from: notifyFrom.top,
			align: notifyAlign.center
		},
		error: {
			icon: notifyIcons.error,
			type: notifyType.error,
			from: notifyFrom.top,
			align: notifyAlign.center
		}
	};
	var notify = function(type, msg, from, align) {
		var ntfo = notifyObject[type];
		from = from ? from : ntfo.from;
		align = align ? align : ntfo.align;
		jQuery.notify({
				icon: ntfo.icon,
				message: msg
			},
	...
	var notifyInfo = function(msg, from, align) {
		notify(notifyType.info, msg, from, align);
	};
	var notifySuccess = function(msg, from, align) {
		...
		notify(notifyType.success, msg, from, align );
	};
	var notifyWarming = function(msg, from, align) {
		notify(notifyType.warming, msg, from, align);
	};
	var notifyError = function(msg, from, align) {
		notify('error', msg, from, align);
	};
	/*
	...
	return {
		...
		loader: function($mode, timeout) {
			uiLoader($mode, timeout);
		},
...
jQuery(function() {
	...
	/*$(window).on("resize",  function(e){
		if(!window._bdoFaithResizeTimeFlag) {
			e.preventDefault();
			e.stopPropagation();
			if(window._bdoFaithResizeTime) {
				return;
				//clearTimeout(window._bdoFaithResizeTime);
			}
			window._bdoFaithResizeTime = setTimeout(()=>{
				window._bdoFaithResizeTimeFlag = true;
				$(e.target).resize();
				clearTimeout(window._bdoFaithResizeTime);
				delete window._bdoFaithResizeTime;
			}, 200);
		}else {
			window._bdoFaithResizeTimeFlag = false;
		}
	});*/
	/*$(document).on('DOMSubtreeModified', '.dataTables_scrollHeadInner', function(e) {
		try {
			let $currentEl = $(e.currentTarget);
			console.info(e, $currentEl.width());
		} catch (e) {
			console.error(e);
		}
	});*/
