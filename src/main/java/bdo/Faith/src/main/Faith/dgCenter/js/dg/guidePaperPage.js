...
var GuidePaperPage = (agrs) => {
	...
	_template = agrs.template || tplLoader('dgCenter/html/dg/guidePaperPage.html');
	...
	listener = () => {
		$('#exportExcelBtn').click((event) => {
			...
			if (!isManager) {
				bdoInfoBox('提示', '非项目负责人无权限导出' + fileName.substring(fileName.lastIndexOf('-') + 1, fileName.lastIndexOf('.')) + '！');
				return;
			}
			transition(() => {
				exportExcel();
