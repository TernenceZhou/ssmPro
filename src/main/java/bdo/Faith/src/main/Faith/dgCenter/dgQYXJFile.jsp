...
<body>
<div>
	<div id="mainContent" class="main-content aside-left"></div>
...
<script>
	$(document).ready(() => {
		...
		if (nodeData) {
			// P005-AU001-审定报表-现金流量表.xlsx
			var fileName = nodeData.extraOptions.fileName;
			var fileIndexId = nodeData.extraOptions.fileIndexId;
			document.title = fileName.replace(fileIndexId, '').substring(1).replace('.xlsx', '');
			window.sys_menuId = nodeData.menuId;
			var $div = $('<iframe id="iframe_qyxj_' + nodeData.extraOptions.workpaperId + '" src="/Faith/dgcenter.do?m=openQYXJFileMain&index=' + nodeData.extraOptions.fileIndexId + '&projectId=' + window.CUR_PROJECTID + '" allowfullscreen="true" scrolling="no" width="100%" height="100%" style="border: none;"></iframe>');
