	...
	items : [
		{
			...
			html : '<br><a class="btn btn-primary" id="selectCustomerBtn" data-input-id="n_customerId"><i class="fa fa-plus-square">&nbsp;选择客户</i></a>'
		}
	]
});
...
/** 新建模态框 新增 */
$('#add_save').on('click',function(evt){
	evt.preventDefault();
	var submitUrl = 'admin/OutUser.add.json';
	...
	var data = {
		...
		param5 : $('#n_customerId').data('customerId')
	};
...
/** 新建模态框 隐藏 */
$('#add_close').on('click',function(evt){
	evt.preventDefault();
	$('#modal_new').modal('hide');
});
	...
	items : [
		{
			...
			html : '<br><a class="btn btn-primary" id="eselectCustomerBtn" data-input-id="e_customerId"><i class="fa fa-plus-square">&nbsp;选择客户</i></a>'
		},{
			id :  'e_mobilePhone',
			type : 'input',
			label : '手机',
...
var userId;
/** 行按钮 修改  */
$('#userManageTable').on('click', 'button[name="rowEdit"]', function(evt) {
	evt.preventDefault();
...
/** 修改模态框 修改 */
$('#edit_save').on('click',function(evt){
	evt.preventDefault();
	var submitUrl = 'admin/OutUser.edit.json';
	...
	var data = {
		...
		param6 : $('#e_customerId').data('customerId'),
		param7 : $('#e_sex').val(),
	...
	if(e_hrLoginId != e_hrLoginId1){
		bdoConfirmBox('系统提示', '你确认要绑定这个【' + e_hrLoginId + '】登录名吗?', function(){
			if(checkHRLoginid(e_hrLoginId)){
				bdoErrorBox('失败', '该用户【' + e_hrLoginId + '】已被绑定！');
...
function checkHRLoginid(e_hrLoginId){
	var returnVal;
	$.ajax({
		url : 'admin/OutUser.checkHRLoginid.json',
		type : 'post',
		async : false,
		data : {
			menuId : window.sys_menuId,
...
/** 修改模态框 关闭 */
$('#edit_close').on('click',function(evt){
	evt.preventDefault();
	$('#modal_edit').modal('hide');
});

/** 行按钮 查看 */
$('#userManageTable').on('click', 'button[name="rowView"]', function(evt) {
	evt.preventDefault();
	$(this).closest('tr').dblclick();
});

/** 行双击 */
$('#userManageTable tbody').on('dblclick', 'tr', function(evt) {
	evt.preventDefault();
	formdataSet($('#userManageTable').DataTable().data()[$(this).closest('tr').index()]);
	$('#edit_save').hide();
...
/** 行按钮 禁用 */
$('#userManageTable').on('click', 'button[name="rowDisable"]', function(evt) {
	evt.preventDefault();
	...
	bdoConfirmBox('系统提示', '你确认要禁用【' + hrLoginId + '】该帐号吗?禁用后该人员将无法登录系统，取消禁用可以在还原功能中进行，确定要禁用吗？', function(){
		$.ajax({
			type : 'post',
			url : 'admin/OutUser.stopUser.json',
			data : {
				menuId : window.sys_menuId,
				param1 : userId
			},
...
/** 行按钮 初始化密码 */
$('#userManageTable').on('click', 'button[name="rowRestore"]', function(evt) {
	evt.preventDefault();
	var userId = $('#userManageTable').DataTable().data()[$(this).closest('tr').index()].id;
	$.ajax({
		type : 'post',
		url : 'admin/OutUser.initPwd.json',
		data : {
			menuId : window.sys_menuId,
			param1 : userId
		},
...
var targetName;
/** 行按钮 授权 */
$('#userManageTable').on('click', 'button[name="rowAuthorize"]', function(evt) {
	evt.preventDefault();
	userId = $('#userManageTable').DataTable().data()[$(this).closest('tr').index()].id;
...
$('#modal_authorize').on('shown.bs.modal', function(){
	...
	/** 搜索按钮 */
	$('#btn_authorizeSearch').click(function(evt) {
		evt.preventDefault();
		tableParam.filterParam.filter = authorizeQueryFilter();
		$('#authorizeTable').DataTable().ajax.reload();
	});
	
	/** 重置按钮 */
	$('#btn_authorizeClear').click(function(evt) {
		evt.preventDefault();
	...
	//	dom : '<"row"<"col-sm-12"tr>>',
		...
		//必需
		sourceData : {},
		sourceUrl : 'admin/Menu.queryOutuserMenuAuth.json',
		filterParam : {
			menuId : window.sys_menuId,
			param1 : '0',
			param3 : 'user',
	...
	/** 行按钮 菜单授权  */
	$('#authorizeTable').on('click', 'button[name="menuAuthorize"]', function(evt) {
		evt.preventDefault();
		...
		if(authed != 'false'){
			...
			$.ajax({
				url : 'admin/Menu.addOutUserMenuAuth.json',
				type : 'post',
				data : {
					param1 : '0',
					param3 : 'user',
	...
	/** 行按钮 菜单解除授权  */
	$('#authorizeTable').on('click', 'button[name="cancleAuthorize"]', function(evt) {
		evt.preventDefault();
		var authed = $('#authorizeTable').DataTable().data()[$(this).closest('tr').index()].authed;
		...
		if(authed != 'true'){
			...
			$.ajax({
				url : 'admin/Menu.removeOutUserMenuAuth.json',
				type : 'post',
				data : {
					param1 : '0',
					param3 : 'user',
	...
	var targetDeptId;
	/** 行按钮 部门授权  */
	$('#authorizeTable').on('click', 'button[name="deptAuthorize"]', function(evt) {
		evt.preventDefault();
		var authed = $('#authorizeTable').DataTable().data()[$(this).closest('tr').index()].authed;
	...
	$('#modal_deptAuthorize').on('shown.bs.modal', function(){
		...
		/** 搜索按钮 */
		$('#btn_deptAuthorizeSearch').click(function(evt) {
			evt.preventDefault();
			tableParam.filterParam.filter = deptQueryFilter();
			$('#deptAuthorizeTableParam').DataTable().ajax.reload();
		});
		
		/** 重置按钮 */
		$('#btn_deptAuthorizeClear').click(function(evt) {
			evt.preventDefault();
...
//			order : [2, 'asc'],
		...
		/** 增加授权 */
		$('#btn_authorizeAdd').click(function(evt){
			evt.preventDefault();
			var selectedDeps = '';
		...
		/** 取消授权 */
		$('#btn_authorizeMove').click(function(evt){
			evt.preventDefault();
			var selectedDeps = '';
			$('input[name="checkAdd"]:checked').each(function(){
				var object = $('#deptAuthorizeTableParam').DataTable().data()[$(this).closest('tr').index()];
		...
		/** 机构内授权 */
		$('#btn_deptAuthorize').click(function(evt){
			evt.preventDefault();
			var selectedDeps = '';
			$('input[name="checkAdd"]:checked').each(function(){
				var object = $('#deptAuthorizeTableParam').DataTable().data()[$(this).closest('tr').index()];
		...
		/** 非机构内全部授权 */
		$('#btn_allAuthorize').click(function(evt){
			evt.preventDefault();
			var data = {
				param1 : '0',
				param3 : 'user',
		...
		/** 取消机构内授权 */
		$('#btn_deptAuthorizeMove').click(function(evt){
			evt.preventDefault();
			var selectedDeps = '';
			$('input[name="checkAdd"]:checked').each(function(){
				var object = $('#deptAuthorizeTableParam').DataTable().data()[$(this).closest('tr').index()];
		...
		/** 取消全部授权 */
		$('#btn_allAuthorizeMove').click(function(evt){
			evt.preventDefault();
			var data = {
				param1 : '0',
				param3 : 'user',
		...
		/** 关闭 */
		$('#deptAuthorizeClose').click(function(evt){
			evt.preventDefault();
			$('#search_mechanismName').val(null);
			$('#search_deptName').val(null);
			$('#search_deptPath').val(null);
	...
	/** 关闭 */
	$('#authorizeClose').click(function(evt){
		evt.preventDefault();
		$('#search_menuPath').val(null);
		$('#search_menuName').val(null);
		$('#search_lowerLevel').val(null);
...
/** 行按钮 人物角色授权 */
$('#userManageTable').on('click', 'button[name="rowRoleAuthorize"]', function(evt) {
	evt.preventDefault();
	userId = $('#userManageTable').DataTable().data()[$(this).closest('tr').index()].id;
	targetName = $('#userManageTable').DataTable().data()[$(this).closest('tr').index()].name;
...
$('#modal_roleAuthorize').on('shown.bs.modal', function(){
	...
	/** 搜索按钮 */
	$('#btn_roleAuthorizeSearch').click(function(evt) {
		evt.preventDefault();
		tableParam.filterParam.filter = roleQueryFilter();
		$('#roleAuthorizeTableParam').DataTable().ajax.reload();
	});
	
	/** 重置按钮 */
	$('#btn_roleAuthorizeClear').click(function(evt) {
		evt.preventDefault();
	...
	$('#roleSelectAll').click(function(evt) {
		evt.preventDefault();
	...
	/** 添加角色权限 */
	$('#btn_roleAuthorizeAdd').click(function(evt){
		evt.preventDefault();
		var roleId = '';
	...
	/** 删除角色权限 */
	$('#btn_roleAuthorizeMove').click(function(evt){
		evt.preventDefault();
		var roleId = '';
		$('input[name="roleCheckAdd"]:checked').each(function(){
			var object = $('#roleAuthorizeTableParam').DataTable().data()[$(this).closest('tr').index()];
		...
		/** 关闭 */
	$('#roleAuthorizeClose').click(function(evt){
...
/** 行按钮 手动同步 */
$('#userManageTable').on('click', 'button[name="rowManual"]', function(evt) {
	evt.preventDefault();
	var hrLoginId = $('#userManageTable').DataTable().data()[$(this).closest('tr').index()].hrLoginId;
	var hrId = $('#userManageTable').DataTable().data()[$(this).closest('tr').index()].hrId;
	$.ajax({
		type : 'post',
		url : 'admin/OutUser.synUserFromHr.json',
		data : {
			menuId : window.sys_menuId,
			param1 : hrLoginId,
			param2 : hrId
...
//添加人员 
$('#selectCustomerBtn,#eselectCustomerBtn').click(function(evt){
	evt.preventDefault();
	$('#modal_customer').modal('show');
	$('#modal_customer').data('fromId', $('#'+evt.currentTarget.id).data('inputId'));
});

//所有人员检索条件  
var customerQueryFilter = function () {
	var queryFilterArr = [];
	if ($('#customer_id').val() != '') {
		queryFilterArr.push({
			'field' : 'customerId',
	...
	tableColumns :[
		{
			...
			render : function(data, type, row, meta){
				var checkBox = '';
				checkBox +='<input type="radio" name="customercheckAdd" value="'+row.customerId+'" data-customer-name="'+row.customerName+'">'
				return checkBox;
			}
		},{
			targets : 2,
...
//添加人员  
$('#btn-customerSelect').click(function(evt){
	evt.preventDefault();
	var id = '#'+$('#modal_customer').data('fromId');
	$(id).data('customerId', $('input[name=customercheckAdd]:checked').val());
	$(id).val($('input[name=customercheckAdd]:checked').data('customerName'));
	$('#modal_customer').modal('hide');
});

//所有人员table  
$('#modal_customer').on('shown.bs.modal', function(){
	customerTableParam.filterParam = {
		sqlId : 'sys.S100205',
		menuId : window.sys_menuId,
		filter : customerQueryFilter
	}
	BdoDataTables('customerTable', customerTableParam);
});
...
function formdataSet(object){
	...
	$('#e_customerId').val(object.__ccustomerIdName);
	$('#e_customerId').data('customerId', object.__ccustomerId.departID);
}