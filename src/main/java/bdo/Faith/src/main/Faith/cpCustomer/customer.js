...
//									App.loader('hide');
									...
									bdoConfirmBox('提示', '是否添加并更新客户信息?', function(){
										App.loader('show');
										$.ajax({
		...
		tableColumns :[
			{
				...
				render : function(data, type, row, meta) {
					var renderStr = '';
					if(row.customerType == 1 && row.isblack == 0) {
			...
			success : function(data){
				...
				if(isNew == "true"){
					...
					if(type == "1"){ // 1：无需更新
						...
						bdoConfirmBox('提示', '客户已存在，是否更新客户信息?', function(){
							App.loader('show');
							$.ajax({
								url : 'cusBase/Customer.getCustomerCheckResult.json',
								type : 'post',
...
//	   $(event.target).find('select:not(#addCus_nationality), select:not(#addCus_customerType)').attr('disabled','disabled');
	...
	$('#addHuoBi').click(function(){
		var tmpRecord = '';
		$('#huobi a').each(function(index,e){
			var title = $(this).attr('title').replace(/汇率/g, "");
			var code = $(this).text();
			tmpRecord = tmpRecord + ',{"title":"' + title + '","code":"' + code + '"}';
		});
		var jsonData = '{"storeMap":[' + tmpRecord.substring(1) + ']}';
		
		$.ajax({
			url : 'cusBase/Customer.addHuoBi.json',
			type : 'post',
			data : {
				param1 : jsonData
			},
