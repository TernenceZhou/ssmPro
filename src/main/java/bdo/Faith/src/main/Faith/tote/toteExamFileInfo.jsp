...
<body>
<div>
	<div id="mainContent" style="height: 100vh"></div>
</div>
...
<script src="${pageContext.request.contextPath}/tote/js/examPaper.js"></script>
<script>
	$(document).ready(() => {
		var nodeData = JSON.parse($.sessionStorage('fileNode'));
		if (nodeData) {
			document.title = nodeData.extraOptions.examName + '-' + nodeData.extraOptions.subjectName;
			window.sys_menuId = nodeData.menuId;
			ExamPaperPage({region: '#mainContent', data: nodeData});
			
			if(nodeData.type == '3'){
				$('#contentBlock .block-header').remove();
			} else if(nodeData.type == '2'){
				document.title = nodeData.extraOptions.name + '-' + nodeData.extraOptions.subjectName;
				$('#contentBlock #finishBtn').attr('title','审核');
				$('#contentBlock #finishBtn i').text(' 审核');
			} else if(nodeData.type == '1'){
				/* if(nodeData.extraOptions.isTest != 1){
					$('#exportExcelBtn').remove();
