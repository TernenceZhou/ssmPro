...
<script>
	$(document).ready(() => {
		if (window.nodeData == null) {
			window.nodeData = JSON.parse($.sessionStorage('dgFileNode'));
			if (window.nodeData == null) {
		...
		if (nodeData) {
			...
			var $div = $('<iframe id="iframe_dg_' + nodeData.extraOptions.workpaperId + '" src="/Faith/dgcenter.do?m=openDgIframeMain&index=' + nodeData.extraOptions.indexId + '&projectId=' + window.CUR_PROJECTID + '" allowfullscreen="true" scrolling="no" width="100%" height="100%" style="border: none;"></iframe>');
