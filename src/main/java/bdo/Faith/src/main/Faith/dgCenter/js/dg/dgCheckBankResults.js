 ...
 * 银行核对结果在线底稿
 */
var DgCheckBankResultsPage = (agrs) => {
	var _template
		, _data
		, currentNode
		, mount
	...
	_template = agrs.template || tplLoader('dgCenter/html/dg/dgCheckBankResults.html');
	...
	var ready = function() {
		...
		designer.paperType = 'bankResults';
	...
	getExcelData = (param) => {
		spread = designer.Spread;
		$.ajax({
			url: 'dgCenter/DgWapper.getBankResults.json',
			type: 'POST',
			data: param,
			dataType: 'json',
			success(data) {
