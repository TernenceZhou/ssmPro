 ...
 * 审计程序页面
 */
function AuditProgramPagePlan(agrs) {
	let _data = agrs.data
			...
			saveChange() {
				...
				promise.then((data) => {
					...
					this.timer = setTimeout(function() {
						//$this = this;
						$.ajax({
							url: 'dgCenter/DgSubjectIdentify.updateIdentifiedNeedAudit.json',
							dataType: 'json',
							data: {
								customerId: window.CUR_CUSTOMERID,
								projectId: window.CUR_PROJECTID,
	...
	var page = new Page({
		...
		, queryGroupMembers() {
			...
			$.ajax({
				...
				success(data) {
					if (data.success) {
						var jsonArr = '';
						me.selectEditors = data.data.map(item => {
							return {id: item.id, text: item.name, value: item.name};
						});
						me.selectEditors = [{id: '', text: '', value: ''}].concat(me.selectEditors);
		...
		onEditAllClick(event) {
			...
			for(let i = 0; i < rowData.length; i++) {
				if (!rowData[i].tbSubject || rowData[i].tbSubject == '') {
		...
		createDgAllClick(event) {
			...
			var textAudit = '<div style="height: 250px;overflow-y: auto"><font color="red">';
			textAudit += '是否同时生成通用财务报表领域及自定义科目审计程序？<br>';
			textAudit += '点击确定将会生成标准和通用领域审计程序<br>';
			textAudit += '点击取消将会只生成标准审计程序</font></div>';
			bdoConfirmBox('提示', textAudit, function() {
		...
		createProjectProgramClick(event) {
			...
			let p = getData('dgCenter/DgGeneral.query.json', {
				menuId: window.sys_menuId,
				sqlId: 'DG00197',
				customerId: window.CUR_CUSTOMERID,
				projectId: window.CUR_PROJECTID,
			...
			p.then((data) => {
				if(CUR_USERID != data.data[0].manager /*&& (
					rowDataI && rowDataI > '' && rowDataI.subjectEditors != CUR_USERID)*/) {
					bdoErrorBox('',"无权限！");
					return new Promise(() => {});
				}
			}, (data) => {
				...
				$('#s_projectId').val('').trigger('change');
				$('#s_customerSubjectId').val('');
				return getData('cpBase/Combo.getCustomerList.json', {
					page: -1,
					start: -1
				});
			}).then((data) => {
				...
				$('#s_customerId').change(function() {
					let pc = getData('cpBase/Combo.getCustomerProjectList.json', {
						param1: $('#s_customerId option:selected').val(),
						page: -1,
						start: -1
					});
		...
		, onNotAuditReasonsBtnClick(event) {
			...
			$.ajax({
				url: 'dgCenter/DgSubjectIdentify.updateIdentifiedReasons.json',
				dataType: 'json',
				data: {
					customerId: window.CUR_CUSTOMERID,
					projectId: window.CUR_PROJECTID,
		...
		, onEditProgramPlan(event) {
			...
			if(rowData.baseSubject == null || rowData.baseSubject == ''){
				bdoErrorBox('操作失败', '请先设置标准科目对照！');
				return;
			}
			let nodeData;
		...
		, OnEditProgramPageTabLinkShownBsTab(evt) {
			...
			if(rowData && rowData > '' && rowData.subjectEditors == sys_userId ) {
				page.editProgramPageNodeData.pageType = null;
				EditProgramPage({region: '#editProgramPageTab', data: page.editProgramPageNodeData});
				return;
			}
			if (rowData) {
				rowData.pageType = null;
			}
			$.ajax({
				type: 'post',
				url: 'dgCenter/DgGeneral.query.json',
		...
		, onCreateDgSub(event) {
			...
			if(rowData.isNeedToAudit != '1'){
				bdoErrorBox('操作失败', '客户科目：【' + rowData.userSubject + '】请先勾选是否编制！');
				return;
			}
			if (rowData.userSubject == null || rowData.userSubject == '' || rowData.tbSubject == null || rowData.tbSubject == '') {
			...
			tableParam: {
				...
				columnDefs: [{
					...
					render(data, type, row, meta) {
						var renderStr = '';
						renderStr += '<div class="">';
						renderStr += '<button class="btn btn-xs btn-info table-btn-operate bdo-drop-btn" type="button" name="createDgSub" data-placement="top" title="生成审计程序" data-toggle="tooltip" data-row="' + meta.row + '">'
							+ '	<i class="fa fa-send"></i>'
							+ '	</button>';
						renderStr += '<button class="btn btn-xs btn-info table-btn-operate bdo-drop-btn" type="button" name="editProgramPlan" data-placement="top" title="编辑审计程序" data-toggle="tooltip" data-row="' + meta.row + '">'
							+ '	<i class="fa fa-edit"></i>'
							+ '	</button>';
						renderStr += '<button class="btn btn-xs btn-danger table-btn-operate bdo-drop-btn" type="button" name="repeatDgSub" data-placement="top" title="重置审计程序" data-toggle="tooltip" data-row="' + meta.row + '">'
