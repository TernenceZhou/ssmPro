...
var TplPage = (agrs) => {
	...
	_template = agrs.template || tplLoader('dgCenter/html/dg/dgTplFile.html');
	agrs.template = _template;
	reportTplId = _data.extraOptions.reportTplId;
	knoteId = _data.extraOptions.autoId;
	noteNo = _data.extraOptions.noteNo;
	if (height <= 0) {
	...
	getExcelData = (param) => {
		spread = designer.Spread;
		$.ajax({
			url: 'dgCenter/DgNote.getNoteTpl.json',
			type: 'POST',
			data: param,
			dataType: 'json',
			success(data) {
