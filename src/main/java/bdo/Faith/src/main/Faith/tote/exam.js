$(document).ready(function() {
			...
			columnDefs: [
				{
					...
					render: function(data, type, row, meta) {
						...
						renderStr += '<button class="btn btn-xs btn-success" type="button" name="examAnalyzeBtn" data-placement="top" title="公布" data-toggle="tooltip"><i class="fa fa-commenting-o"></i></button>&nbsp;';
	...
	//试卷分析
	$('#examTable').on('click', 'button[name="examAnalyzeBtn"]', function() {
		var object = $('#examTable').DataTable().data()[$(this).closest('tr').index()];
		var tip = '确认开始公布试卷？';
		bdoConfirmBox('确认', tip, function() {
			$.ajax({
				url: 'tote/ToteExam.analyzeExam.json',
				type: 'post',
				data: {
					param1: object.autoId
				},
