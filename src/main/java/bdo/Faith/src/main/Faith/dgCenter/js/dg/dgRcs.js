function RcsPage(agrs) {
	let _template = agrs.template || tplLoader('dgCenter/html/dg/dgRcs.html')
		, customerId = agrs.data.customerId
		, projectId = agrs.data.projectId;
	$(agrs.region).html(_template);
	var page = new Page({
		container: '#subPageRight',
		events: {
		...
		uploadWorkpagerPageCfg: {
			...
			data() {
				return {
					ajaxConfig: {
						type: 'POST',
						url: 'dgCenter/DgDownload.uploadResFile.json',
						dataType: 'json',
						success(data) {
							if (data.success) {
		...
		folderTreeOptions: {
			url: './cpBase/TreeCommon.findDgRcsFileSystemTree.json',
			params: {
				param13: '1',
				param12: '01',
				param22: customerId,
		...
		loadFolderTreeTable: function(params) {
			...
			easyloader.load(['datagrid'], function() {
				...
				folderTreeTable.datagrid('getPanel').on('click', 'a[data-watch]', me.onWatchFileClick.bind(me));
			});
		},
		...
		onEditFileClick: function(evt) {
			...
			if (fsId) {
				...
				if (rowData) {
					me.cmp.folderTreeTable.datagrid('unselectRow', rowData.autoId);
				}
		...
		onWatchFileClick: function(evt) {
			...
			if ($this.attr('data-watch')) {
				$.ajax({
					url: 'dgCenter/DgMain.queryResFileExistence.json',
					type: 'post',
					data: {
						customerId: window.CUR_CUSTOMERID,
						projectId: window.CUR_PROJECTID,
					...
					success: function(data) {
						if (data.resultInfo.status == '3') {
							...
							window.open('dgCenter/DgMain.previewResFile.json?param1=' + $this.attr('data-watch') + '&param2=' + customerId, '预览', 'location=no');
						}
					}
				});
			}
		...
		deleteFile: function(fsId) {
			...
			var tv = me.cmp.folderTree.treeview(true);
			$.ajax({
				url: 'dgCenter/DgMain.deleteResFolder.json',
				type: 'post',
				data: {
					customerId: window.CUR_CUSTOMERID,
					projectId: window.CUR_PROJECTID,
		...
		onDownloadFileClick: function(evt) {
			...
			if ($this.attr('data-download')) {
				me.downloadFile('dgCenter/DgDownload.downloadResFile.json', {
					customerId: window.CUR_CUSTOMERID,
					projectId: window.CUR_PROJECTID,
		...
		onAddSaveBtnClick: function(evt) {
			evt.preventDefault();
			var me = this;
			var submitUrl = 'dgCenter/DgMain.addRcsFolder.json';
			var tv = me.cmp.folderTree.treeview(true);
			var parentId = 0;
		...
		onEditSaveBtnClick: function(evt) {
			evt.preventDefault();
			var me = this;
			var submitUrl = 'dgCenter/DgMain.updateResFolder.json';
