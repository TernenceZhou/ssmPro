...
<script src="${pageContext.request.contextPath}/dgCenter/js/dg/lastXJQYFile.js"></script>

<script>
	$(document).ready(() => {
		var nodeData = JSON.parse($.sessionStorage('excelxjqynode'));
		if (nodeData) {
			window.sys_menuId = nodeData.menuId;
			LastXJQYFilePage({region: '#mainContent2', data: nodeData});
