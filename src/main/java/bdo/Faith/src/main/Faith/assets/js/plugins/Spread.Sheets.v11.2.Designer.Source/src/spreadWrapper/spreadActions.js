(function () {
	...
	// 系统剪贴板异步读写（需要用户允许）
	let SysClipboard = Object.freeze({
		setText: str => navigator.clipboard && navigator.clipboard.writeText && navigator.clipboard.writeText(str).catch(err => {}), // 失败时无视
		getText: () => new Promise((resolve, reject) => {
			if (navigator.clipboard && navigator.clipboard.readText) {
				navigator.clipboard.readText().then(rs => resolve(rs), err => resolve('')); // 失败时返回空串
			} else {
				resolve(''); // 不支持时返回空串
			}
		})
    ...
    function initDesignerCommands(commandManager) {
        ...
        /* ZQF START */
        //cellType
        commandManager.register(MENUITEM_NAME_PREFIX + CELL_TYPE, Commands[CELL_TYPE], null, false, false, false, false);
        //customLabel
        commandManager.register(MENUITEM_NAME_PREFIX + CUSTOM_LABEL, Commands[CUSTOM_LABEL], null, false, false, false, false);
        //customLinks
        // commandManager.register(MENUITEM_NAME_PREFIX + CUSTOM_LINKS, Commands[CUSTOM_LINKS], null, false, false, false, false);
        //mutualIndex
        commandManager.register(MENUITEM_NAME_PREFIX + MUTUAL_INDEX, Commands[MUTUAL_INDEX], null, false, false, false, false);
        //leftTag
        commandManager.register(MENUITEM_NAME_PREFIX + LEFT_TAG, Commands[LEFT_TAG], null, false, false, false, false);
        //rightTag
        commandManager.register(MENUITEM_NAME_PREFIX + RIGHT_TAG, Commands[RIGHT_TAG], null, false, false, false, false);
        //noteFeature
        commandManager.register(MENUITEM_NAME_PREFIX + NOTE_FEATURE, Commands[NOTE_FEATURE], null, false, false, false, false);
        //cashFlowFeature
        commandManager.register(MENUITEM_NAME_PREFIX + CASHFLOW_FEATURE, Commands[CASHFLOW_FEATURE], null, false, false, false, false);
        //customClear
        commandManager.register(MENUITEM_NAME_PREFIX + CUSTOM_CLEAR, Commands[CUSTOM_CLEAR], null, false, false, false, false);
        /* ZQF END */
        //rowHeight
    ...
    var CopyAction = (function (_super) {
        ...
        CopyAction.prototype.executeImp = function (context, options, isUndo) {
			...
			SysClipboard.setText(command.copyData.copyText);
			localStorage.setItem("command.copyData.copyText", command.copyData.copyText);
			localStorage.setItem("command.copyData.copyHtml", command.copyData.copyHtml);
        };
        return CopyAction;
    })(DesignerActionBase);
    Commands[COPY] = {
    ...
    var CutAction = (function (_super) {
        ...
        CutAction.prototype.executeImp = function (context, options, isUndo) {
            ...
            command.callback = callback;
            spread.commandManager().execute(command);
			SysClipboard.setText(command.cutData.copyText);
			localStorage.setItem("command.copyData.copyText", command.copyData.copyText);
			localStorage.setItem("command.copyData.copyHtml", command.copyData.copyHtml);
        };
        return CutAction;
    ...
    var PasteAction = (function (_super) {
        ...
        PasteAction.prototype.executeImp = async function (context, options, isUndo) {
            var self = this;
            ...
            if (sheet.isPasteFloatingObject()) {
				...
				// 系统剪贴板内容
                let sysClipText = await SysClipboard.getText();
				if (sysClipText && sysClipText != localStorage.getItem("command.copyData.copyText")){
				    // 系统剪贴板有内容且和spreadjs剪贴板内容不一样，是用户又更新了系统剪贴板，这时清空spreadjs剪贴板
					designer.util.clipboard.copyText = '';
					designer.util.clipboard.copyHtml = '';
					localStorage.removeItem("command.copyData.copyText");
					localStorage.removeItem("command.copyData.copyHtml");
				}

				result = sysClipText || localStorage.getItem("command.copyData.copyText");   // 优先使用系统剪贴板内容(仅丢弃格后的文本)
				//clipboardHtml = designer.util.clipboard.copyHtml || result; // 没有格式时直接用内容
				clipboardHtml = localStorage.getItem("command.copyData.copyHtml") || result; // 没有格式时直接用内容，使用localStorage以支持多窗口间复制
				options.value == 2 && (result = '');                        // 粘贴格式时，清空内容
				options.value == 3 && sysClipText && !localStorage.getItem("command.copyData.copyText") && (result = ''); // 使用系统剪贴板粘贴公式时，令无效果
...
/* ZQF START */
    var CellTypeAction = (function (_super) {
        designer.extends(CellTypeAction, _super);

        function CellTypeAction(spread, options) {
            _super.call(this, spread, options);
        }

        CellTypeAction.prototype.executeImp = function (context, options, isUndo) {
            if (designer && designer.dialog) {
                if (designer.dialog.CellTypeDialog === undefined) {
                    designer.dialog.CellTypeDialog = new designer.CellTypeDialog();
                }
                designer.dialog.CellTypeDialog.open();
            }
        };
        return CellTypeAction;
    })(DesignerActionBase);
    Commands[CELL_TYPE] = {
        canUndo: false,
        execute: function (context, options, isUndo) {
            var cmd = new CellTypeAction(context, options);
            return cmd.execute(context, options, isUndo);
        }
    };

    var CustomLabelAction = (function (_super) {
        designer.extends(CustomLabelAction, _super);

        function CustomLabelAction(spread, options) {
            _super.call(this, spread, options);
        }

        CustomLabelAction.prototype.executeImp = function (context, options, isUndo) {
            if (designer && designer.dialog) {
                if (designer.dialog.CustomLabelDialog === undefined) {
                    designer.dialog.CustomLabelDialog = new designer.CustomLabelDialog();
                }
                designer.dialog.CustomLabelDialog.open();
            }
        };
        return CustomLabelAction;
    })(DesignerActionBase);
    Commands[CUSTOM_LABEL] = {
        canUndo: false,
        execute: function (context, options, isUndo) {
            var cmd = new CustomLabelAction(context, options);
            return cmd.execute(context, options, isUndo);
        }
    };

    var MutualIndexAction = (function (_super) {
        designer.extends(MutualIndexAction, _super);

        function MutualIndexAction(spread, options) {
            _super.call(this, spread, options);
        }

        MutualIndexAction.prototype.executeImp = function (context, options, isUndo) {
            if (designer && designer.dialog) {
                if (designer.dialog.MutualIndexDialog === undefined) {
                    designer.dialog.MutualIndexDialog = new designer.MutualIndexDialog();
                }
                designer.dialog.MutualIndexDialog.open();
            }
        };
        return MutualIndexAction;
    })(DesignerActionBase);
    Commands[MUTUAL_INDEX] = {
        canUndo: false,
        execute: function (context, options, isUndo) {
            var cmd = new MutualIndexAction(context, options);
            return cmd.execute(context, options, isUndo);
        }
    };

    /*var CustomLinksAction = (function (_super) {
        designer.extends(CustomLinksAction, _super);

        function CustomLinksAction(spread, options) {
            _super.call(this, spread, options);
        }

        CustomLinksAction.prototype.executeImp = function (context, options, isUndo) {
            if (designer && designer.dialog) {
                if (designer.dialog.CustomLinksDialog === undefined) {
                    designer.dialog.CustomLinksDialog = new designer.CustomLinksDialog();
                }
                designer.dialog.CustomLinksDialog.open();
            }
        };
        return CustomLinksAction;
    })(DesignerActionBase);
    Commands[CUSTOM_LINKS] = {
        canUndo: false,
        execute: function (context, options, isUndo) {
            var cmd = new CustomLinksAction(context, options);
            return cmd.execute(context, options, isUndo);
        }
    };*/

    var NoteFeatureAction = (function (_super) {
        designer.extends(NoteFeatureAction, _super);

        function NoteFeatureAction(spread, options) {
            _super.call(this, spread, options);
        }

        NoteFeatureAction.prototype.executeImp = function (context, options, isUndo) {
            if (designer && designer.dialog) {
        ...
        return NoteFeatureAction;
    })(DesignerActionBase);
    Commands[NOTE_FEATURE] = {
        canUndo: false,
        execute: function (context, options, isUndo) {
            var cmd = new NoteFeatureAction(context, options);
            return cmd.execute(context, options, isUndo);
        }
    };

    var CashFlowFeatureAction = (function (_super) {
        designer.extends(CashFlowFeatureAction, _super);

        function CashFlowFeatureAction(spread, options) {
            _super.call(this, spread, options);
        }

        CashFlowFeatureAction.prototype.executeImp = function (context, options, isUndo) {
            if (designer && designer.dialog) {
        ...
        return CashFlowFeatureAction;
    })(DesignerActionBase);
    Commands[CASHFLOW_FEATURE] = {
        canUndo: false,
        execute: function (context, options, isUndo) {
            var cmd = new CashFlowFeatureAction(context, options);
            return cmd.execute(context, options, isUndo);
        }
    };

    var CustomClearAction = (function (_super) {
        designer.extends(CustomClearAction, _super);

        function CustomClearAction(spread, options) {
            _super.call(this, spread, options);
        }

        CustomClearAction.prototype.executeImp = function (context, options, isUndo) {
    ...
    Commands[CUSTOM_CLEAR] = {
        canUndo: false,
        execute: function (context, options, isUndo) {
            var cmd = new CustomClearAction(context, options);
            return cmd.execute(context, options, isUndo);
        }
    };

    var LeftTagAction = (function (_super) {
        designer.extends(LeftTagAction, _super);

        function LeftTagAction(spread, options) {
            _super.call(this, spread, options);
        }

        LeftTagAction.prototype.executeImp = function (context, options, isUndo) {
            var tagValue = context.getActiveSheet().getValue(options.activeRow, options.activeCol);
            if(tagValue== null || tagValue == ''){
                tagValue = 0;
            }
            var range = new GC.Spread.Sheets.Range(options.activeRow, options.activeCol, 1, 1);
            ...
            if(designer.paperType == 'note'){
                autoId = designer.notePaperId;
            }else if(designer.paperType = 'dg'){
                autoId = designer.workpaperId;
            }
            var tagName = autoId + ':' + options.sheetName + ':' + rangeStr;
            var tagPosition = tagName;
            var tagInfo = [];
            if(designer.paperType == 'note'){
                tagInfo.push({
                    type: 'note',
            ...
            if($("#right_tagPosition", window.parent.document).val() != null && $("#right_tagPosition", window.parent.document).val() != ''){
                $.ajax({
                	url : 'dgCenter/DgCheck.mutualIndexFormula.json',
                    // async : false,
                    type : 'post',
                    data : {
                        ...
                        param9: $("#right_tagName", window.parent.document).val(),
                        param10: $("#right_tagPosition", window.parent.document).val(),
                        param11: $("#right_tagValue", window.parent.document).val(),
                        param12: $("#right_tagInfo", window.parent.document).val(),
                        param13: $("#right_tagDgId", window.parent.document).val(),
                        param14: $("#right_tagType", window.parent.document).val()
                    },
                    dataType : 'json',
                    success : function(data){
                        if(data.success == true) {
                            $("#right_tagName", window.parent.document).val('');
                            $("#right_tagPosition", window.parent.document).val('');
                            $("#right_tagValue", window.parent.document).val('');
                            $("#right_tagInfo", window.parent.document).val('');
                            $("#right_tagDgId", window.parent.document).val('');
                            $("#right_tagType", window.parent.document).val('');
                            $("#tagLabel", window.parent.document).html('成功设置校验公式！');
                        }
                    }
                });
            }else{
                $("#left_tagName", window.parent.document).val(tagName);
                $("#left_tagPosition", window.parent.document).val(tagPosition);
                $("#left_tagValue", window.parent.document).val(tagValue);
                $("#left_tagInfo", window.parent.document).val(JSON.stringify(tagInfo));
                $("#left_tagDgId", window.parent.document).val(autoId);
                $("#left_tagType", window.parent.document).val(designer.paperType);
                $("#tagLabel", window.parent.document).html('已设置左标签');
            }
        };
        return LeftTagAction;
    })(DesignerActionBase);
    Commands[LEFT_TAG] = {
        canUndo: false,
        execute: function (context, options, isUndo) {
            var cmd = new LeftTagAction(context, options);
            return cmd.execute(context, options, isUndo);
        }
    };

    var RightTagAction = (function (_super) {
        designer.extends(RightTagAction, _super);

        function RightTagAction(spread, options) {
            _super.call(this, spread, options);
        }

        RightTagAction.prototype.executeImp = function (context, options, isUndo) {
            var tagValue = context.getActiveSheet().getValue(options.activeRow, options.activeCol);
            if(tagValue== null || tagValue == ''){
                tagValue = 0;
            }
            ...
            if($("#left_tagPosition", window.parent.document).val() != null && $("#left_tagPosition", window.parent.document).val() != ''){
                $.ajax({
                    url : 'dgCenter/DgCheck.mutualIndexFormula.json',
                    // async : false,
                    type : 'post',
                    data : {
                        ...
                        param1: window.CUR_CUSTOMERID,
                        param2: window.CUR_PROJECTID,
                        param3: $("#left_tagName", window.parent.document).val(),
                        param4: $("#left_tagPosition", window.parent.document).val(),
                        param5: $("#left_tagValue", window.parent.document).val(),
                        param6: $("#left_tagInfo", window.parent.document).val(),
                        param7: $("#left_tagDgId", window.parent.document).val(),
                        param8: $("#left_tagType", window.parent.document).val(),
                        param9: tagName,
                        param10: tagPosition,
                        param11: tagValue,
                        param12: JSON.stringify(tagInfo),
                        param13: autoId,
                        param14: designer.paperType
                    },
                    dataType : 'json',
                    success : function(data){
                        if(data.success == true) {
                            $("#left_tagName", window.parent.document).val('');
                            $("#left_tagPosition", window.parent.document).val('');
                            $("#left_tagValue", window.parent.document).val('');
                            $("#left_tagInfo", window.parent.document).val('');
                            $("#left_tagDgId", window.parent.document).val('');
                            $("#left_tagType", window.parent.document).val();
                            $("#tagLabel", window.parent.document).html('成功设置校验公式！');
                        }
                    }
                });
            }else{
                $("#right_tagName", window.parent.document).val(tagName);
                $("#right_tagPosition", window.parent.document).val(tagPosition);
                $("#right_tagValue", window.parent.document).val(tagValue);
                $("#right_tagInfo", window.parent.document).val(JSON.stringify(tagInfo));
                $("#right_tagDgId", window.parent.document).val(autoId);
                $("#right_tagType", window.parent.document).val(designer.paperType);
                $("#tagLabel", window.parent.document).html('已设置右标签');
            }
        };
        return RightTagAction;
    })(DesignerActionBase);
    Commands[RIGHT_TAG] = {
        canUndo: false,
        execute: function (context, options, isUndo) {
            var cmd = new RightTagAction(context, options);
            return cmd.execute(context, options, isUndo);
        }
    };
/* ZQF END */
    ...
    Commands[FORMAT_CHART] = {
        ...
        execute: function (context, options, isUndo) {
            ...
            designer.ribbon.chartClicked(null, options.commandOptions);
        }
    };
    ...
    var FreezeAction = (function (_super) {
        ...
        FreezeAction.prototype.executeImp = function (context, options, isUndo) {
            var self = this;
            // 冻结窗格 冻结首行 冻结首列
            if (options.value.row !== undefined || options.value.col !== undefined) {
                var frozenRowNum = self._sheet.getActiveRowIndex();
                var frozenColNum = self._sheet.getActiveColumnIndex();
                executeFrozen(self._sheet, frozenRowNum, frozenColNum);
            }
    ...
    // 行隐藏状态缓存
    var rowVisibleStatus = [];
    // 列隐藏状态缓存
    var colVisibleStatus = [];
    function executeFrozen(sheet, frozenRowNumInt, frozenColNumInt) {
        // 没有cancel frozen时，需要先执行cancel
        if(rowVisibleStatus.length > 0 || colVisibleStatus.length > 0){
            cancelFrozen(sheet);
        }
        // 顶部行
        ...
        if(topRow <= frozenRowNumInt && frozenRowNumInt <= bottomRow){
            // 先缓存行的隐藏状态
            cacheRowVisible(sheet, topRow);
            // 设置隐藏行数
            sheet.suspendPaint();
            for(let i = 0; i < topRow;i++){
                sheet.setRowVisible(i, false, GC.Spread.Sheets.SheetArea.viewport);
            }
            // 执行冻结行操作
            sheet.frozenRowCount(frozenRowNumInt);
            sheet.resumePaint();
        }
        // 左侧列
        ...
        if(leftCol <= frozenColNumInt && frozenColNumInt <= rightCol){
            // 先缓存列的隐藏状态：
            cacheColVisible(sheet, leftCol);
            // 设置隐藏列数
            sheet.suspendPaint();
            for(let i = 0; i < leftCol;i++){
                sheet.setColumnVisible(i, false, GC.Spread.Sheets.SheetArea.viewport);
            }
            // 执行冻结列操作
            sheet.frozenColumnCount(frozenColNumInt);
            sheet.resumePaint();
        }
    }
    function cancelFrozen(sheet){
        sheet.suspendPaint();
        ...
        sheet.showColumn(0);
        if(rowVisibleStatus.length > 0 || colVisibleStatus.length > 0){
            for(let i = 0; i < rowVisibleStatus.length;i++){
                sheet.setRowVisible(i, rowVisibleStatus[i], GC.Spread.Sheets.SheetArea.viewport);
            }
            for(let i = 0; i < colVisibleStatus.length;i++){
                sheet.setColumnVisible(i, colVisibleStatus[i], GC.Spread.Sheets.SheetArea.viewport);
            }
            rowVisibleStatus = [];
            colVisibleStatus = [];
        }
    ...
    // 缓存行的隐藏状态
    function cacheRowVisible(sheet, rolNum){
        for(let i = 0; i < rolNum;i++){
            rowVisibleStatus.push(sheet.getRowVisible(i, GC.Spread.Sheets.SheetArea.viewport));
        }
    }
    // 缓存列的隐藏状态
    function cacheColVisible(sheet, colNum){
        for(let i = 0; i < colNum;i++){
            colVisibleStatus.push(sheet.getColumnVisible(i, GC.Spread.Sheets.SheetArea.viewport));
        }
    }
    ...
    var setChartStyleAction = (function (_super) {
        ...
        SetChartStyleAction.prototype.executeImp = function (context, options, isUndo) {
            try {
                var sheet = this.sheet();
                var chart = sheet.charts.get(options.chartName);
                var styles = options.chartStyle;
                var chartAreaStyle, colorAndStyle;
                for (var styleType in styles) {
                    if (styles.hasOwnProperty(styleType)) {
                        if (styleType === "series") {
                            for (var i = 0; i < styles[styleType].length; i++) {
                                var newSeries = styles[styleType][i];
                                chart.series().set(i, newSeries);
                            }
                        } else if (styleType === "chartArea") {
                            chartAreaStyle = styles[styleType];
                ...
                if (chartAreaStyle) {
                    chart.chartArea(chartAreaStyle);
                }
                if (colorAndStyle) {
    ...
    var updateChartSeriesAction = (function (_super) {
        ...
        updateChartSeriesAction.prototype.executeImp = function (context, options, isUndo) {
            ...
            var chart = sheet.charts.get(options.chartName);
            if (chart) {
                chart.series().set(options.series.index, options.series.value);
            }
            triggerChartChanged(chart);
        };
    ...
    var addAndSetStyleByName = function (context, options, isUndo) {
        ...
        this.execInSelections(this.sheet(), function (sheet, row, column) {
            sheet.setStyleName(row, column, styleName);
        });
    };
