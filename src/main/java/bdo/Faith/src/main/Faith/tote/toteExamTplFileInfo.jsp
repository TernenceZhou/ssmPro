...
<script src="${pageContext.request.contextPath}/tote/js/examTplPaper.js"></script>
<script>
	$(document).ready(() => {
		var nodeData = JSON.parse($.sessionStorage('tplFileNode'));
		if (nodeData) {
			document.title = nodeData.extraOptions.subjectName;
			window.sys_menuId = nodeData.menuId;
			ExamTplPaperPage({region: '#mainContent', data: nodeData});
		}
	});
