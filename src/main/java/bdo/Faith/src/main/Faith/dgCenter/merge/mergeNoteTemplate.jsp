...
<script src="${pageContext.request.contextPath}/dgCenter/merge/js/mergeNoteTemplate.js"></script>

<script>
	$(document).ready(() => {
		if (window.nodeData == null) {
			window.nodeData = $.sessionStorage('mergeNoteTemplateNode');
		}
		if (nodeData) {
			var fileName = nodeData.extraOptions.fileName;
			document.title = fileName.replace('.xlsx', '');
			window.sys_menuId = nodeData.menuId;
			MergeNoteTemplatePage({region: '#mainContent', data: nodeData});
		}
	});
</script>
</body>
