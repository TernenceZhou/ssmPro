 ...
 */
							...
							if (me.dgProjectInfo.projectType == '2') {
 ...
 */
			...
			// 合并项目测试临时注释单体账套的不能用合并的报表模板
			/*
			 * if
			 * ($('#tbReportTemplate').val().indexOf(projectInfo.financeReportTypeStr) ==
			 * -1) { bdoInfoBox("提示","已选报表模板与项目财务报表类型不一致!");
			 *  } else {
			 */
				...
				if (!$('#mergeType').prop('checked')){
					...
					if(projectReportTemplate && projectReportTemplate != $('#tbReportTemplate').attr('datavalue')){
						if(projectEndMonth != $('#endMonth').val()){
							displaytext = '更改报表模板、账套结束月份将<br><font color="red">重新对照科目<br>重新生成试算平衡表<br>重新生成未审报表<br>删除全部附注</font><br>您确定继续保存吗？';
							
						}else{
							displaytext = '更改报表模板将<br><font color="red">重新对照科目<br>重新生成试算平衡表<br>重新生成未审报表<br>删除全部附注</font><br>您确定继续保存吗？';
						}
					}else{
						if(projectEndMonth != $('#endMonth').val()){
							displaytext = '更改账套结束月份将<br><font color="red">重新对照科目<br>重新生成试算平衡表<br>重新生成未审报表</font><br>您确定继续保存吗？';
						}
					}
					if(displaytext != ''){
