...
var DgPreViewFilePage = (agrs) => {
	...
	_template = agrs.template || tplLoader('dgCenter/html/dg/dgPreviewFile.html');
	agrs.template = _template;

	customerId = _data.extraOptions.customerId;
	projectId = _data.extraOptions.projectId;
	workpaperId = _data.extraOptions.autoId;
	fullPath = _data.extraOptions.fullPath;
	pageType = _data.extraOptions.pageType;
	...
	getExcelData = (param) => {
		...
		if(pageType == 4){
			$.ajax({
				url: 'dgCenter/DgProjectAssert.getFile.json',
				type: 'POST',
				data: param,
				dataType: 'json',
				success(data) {
			...
			$.ajax({
				url: 'dgCenter/DgAttach.getAttach.json',
				type: 'POST',
				data: param,
				dataType: 'json',
				success(data) {
	...
	downloadExcel = () => {
		...
		if(pageType == 4){
			...
			downloadFile('dgCenter/DgAttach.downloadAttach.json', {
				customerId: window.CUR_CUSTOMERID,
				projectId: window.CUR_PROJECTID,
				param1: fullPath,
				param2: workpaperId,
