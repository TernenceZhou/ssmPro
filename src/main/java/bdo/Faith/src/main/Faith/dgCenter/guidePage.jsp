...
<script src="${pageContext.request.contextPath}/dgCenter/js/dg/guidePaperPage.js"></script>
...
<script>
	$(document).ready(() => {
		...
		if (nodeData) {
			...
			fileName = fileName.replace(nodeData.extraOptions.indexId + '-', '');
			if(fileName == '未审报表'){
				if(nodeData.extraOptions.indexId == 'P001-R002'){
					// 本期
					fileName = fileName + '本期';
				}else{
					// 上期
					fileName = fileName + '上期';
				}
			}
			document.title = fileName;
			GuidePaperPage({region: '#mainContent', data: nodeData});
		}
	});
</script>
</body>
