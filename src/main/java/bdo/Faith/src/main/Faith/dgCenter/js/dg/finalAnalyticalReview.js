FinalAnalyticalReview = (agrs) => {
	...
	let createColumnZC = (i) => {
		...
		tbColumns.columnDefs.push({
			...
			className: 'text-left',
			title: '变动原因',
			name: 'finallyChangeReason',
			data: 'finallyChangeReason',
			width: '300px'
			/*            render: function (data, type, row, meta) {
							return '';
						}*/
	...
	let initFunc = () => $.ajax({
		url: 'dgCenter/FUnAuditReport.finalAnalyticalReview.json',
		type: 'post',
		data: {
			customerId: window.CUR_CUSTOMERID,
			projectId: window.CUR_PROJECTID,
	...
	mount = () => {
		...
		initFunc();
		$('#ar_title').text('最终分析性复核');
		let index = '   <span>【' + agrs.type + '】</span>';
		$('#ar_title').append(index);
	...
	$('#zc_report_table,#lr_report_table').on('dblclick', 'td', function () {
		...
		input.on('blur', function () {
			...
			data['finallyChangeReason'] = value;
	...
	$('#rpt_check_analysisReview').on('click', function () {
		...
		bdoAjaxBox('系统提示', '确定完成核对吗？', function () {
			$.ajax({
				url: 'dgCenter/FUnAuditReport.calculateFinalAnalyticalReview.json',
				type: 'post',
				data: {
					customerId: window.CUR_CUSTOMERID,
					projectId: window.CUR_PROJECTID,
	...
	var checkRequired = function(){
		...
		for(var i=0;i<zc_len;i++){
			...
			if (row.assetsValue < Math.abs(row.assVal) && row.changeThreshold < Math.abs(row.proportion)) {
				if(row.finallyChangeReason == null || row.finallyChangeReason == ''){
	...
	let page = new Page({
		...
		, uploadWorkpagerPageCfg: {
			...
			methods: {
				onUploadDraftFileClick(event) {
					...
					this.uploadFile(true, { param1: agrs.data.type, param11: analysisTypeFinal, param12: uploadChartIdFinal });
				},
		...
		unusual_search() {
			...
			$('#projectId').val(CUR_PROJECTID);
			$('#unusual_slpk').val(CUR_CUSTOMERID);
	...
	var uploadChartIdFinal;
	let actionUnusualFinal = (param, callback) => {
		$.ajax({
	...
	let queryAnalysisReasonFinal = () => {
		$.ajax({
			...
			data: {
				...
				param1: window.CUR_CUSTOMERID,
				param2: window.CUR_PROJECTID,
				param3: analysisTypeFinal,
				param4: uploadChartIdFinal
			},
			dataType: 'json',
			success(data) {
				if (data.success) {
					if (data.data.length > 0) {
						...
						$('#analysisFile' + uploadChartIdFinal).html(fileUl);
					}
				} else {
					bdoErrorBox('失败', data.resultInfo.statusText);
				}
	...
	let calculateFinalAnalysisReview = () => {
		var myDate = new Date();
		var year = window.CUR_PROJECT_ACC_YEAR;//年
		var periodStandard = '2';
		var lastyear = (year - 1) + '-12-31';
						...
						$('.js-tree-collapsed').trigger('rebuild', [{
							...
							callback(tree) {
								$.each(tree.findNodes(), (index, tnode) => {
									if (tnode.extraOptions.nodeType == 'PAR') {
