...
//	});
	...
	}else if(window.hasSacpCer != true && window.systemId == 'SACP') {
	...
	}else if(window.sacpCerExpired > 10 && window.sacpCerExpired <= 30 && window.systemId == 'SACP') {
		//$('#examCerExpired').html('证书即将到期，请前往考试。');
		let msg = '\t证书即将到期，请前往考试。';
		bdoInfoBox('提示', msg);
	}else if(window.sacpCerExpired >= 0 && window.sacpCerExpired <= 10 && window.systemId == 'SACP') {
