...
(function($, window, document, undefined) {
		...
		if (node.nodes && node.nodes.length > 0) {
			for (var x in node.nodes) {
				if (node.state.checked == false) {
					if (x < node.nodes.length && node.nodes[x].state.checked == true) {
						count++;
						tsCopy.push(node.nodes[x].nodeId);
					}
					if (node.nodes.length == count) {
						ts = tsCopy;
					}
					if (x < node.nodes.length && node.nodes[x].state.checked == false) {
	...
	function setParentNodeCheck(node, doc, flag) {
		...
		var parentNode = doc.treeview(true).getParent(node);//doc.treeview(true).getNodeById() getNodeById(nodes, parentId);
...
//根据id获得node bootstrap -- treeview doc.treeView("getNode",nodeId)返回的不是node对象而是函数
	...
	var Tree = function(element, options) {
		...
		this.expandByNode(options.expandNode);
		if (!options.isExpand) {
			this.reset();
		}
		return {
			...
			expandByNode: $.proxy(this.expandByNode, this)
		};
	};
	...
	Tree.prototype.buildTree = function() {
		...
		treeDataJson = this.getTreeData(this.options); // 获取原始Json数据
		if (treeDataJson && treeDataJson.length && treeDataJson.length > 0) {
			...
			$(treeDataJson).each(function(index) {
				...
				if (this.leaf) {
					...
					if (_this.options.lazyLoad2 === false) {
			...
			viewConf.onNodeUnchecked = function(event, node) { //取消选中节点
				...
				//子节点全不选后父节点不选中
				setParentNodeCheck(node, $(this), false);
			};
				...
				.keyup(
					function() {
						...
						if (chiPattern.test(searchVal.trim())
							...
							//重新展开所有目录
							//$("#" + _this.elementId).treeIndustry("reInit");
						}
						$('#' + _this.elementId).treeview('search', [searchVal]);
					});
		}
	};
	...
	Tree.prototype.expandedFun = function(event, node) {
		...
		var lazyLoad2 = me.options.lazyLoad2;
		var parentId = node.parentId;
		if (lazyLoad2 === false) {
			return;
		}
	...
	Tree.prototype.expandByNode = function(expandNode) {

		if (expandNode == null) {
			return;
		}
		var ss = expandNode.split(',');
		//获取子节点
		var arr = [];
		for (var i = 0; i < ss.length; i++) {
			arr.push(ss[i]);
			console.log(ss[i] + '字节点');
		}
		//用来截取子节点获取根节点
		var nodes = [];
		for (var j = 0; j < arr.length; j++) {
			nodes.push(arr[j].substring(0, 4));
		}
		//获取去重后的根节点
		var newArr = [];
		for (var i = 0; i < nodes.length; i++) {
			var check = nodes[i];
			if ($.inArray(check, newArr) == -1) {
				newArr.push(check);
				console.log(check + '去重');
			}
		}
		var parentnode = $('#' + this.elementId).treeview('getEnabled');
		for (var i = 0; i < parentnode.length; i++) {
			for (var j = 0; j < newArr.length; j++) {
				if (parentnode[i].value == newArr[j]) {
					$('#' + this.elementId).find('li[data-nodeid="' + parentnode[i].nodeId + '"]').children('.expand-icon').click();
					var subnode = $('#' + this.elementId).treeview('getEnabled');
					for (var m = 0; m < subnode[i].nodes.length; m++) {
						for (var k = 0; k < arr.length; k++) {
							if (subnode[i].nodes[m].value == arr[k]) {
								$('#' + this.elementId).treeview('checkNode', [subnode[i].nodes[m].nodeId, {
									levels: 2,
									silent: true
								}]);
							}
